...
function doGet(e) {
  ...
  if (action === 'check_auth') {
    ...
    const idx = {
      ...
      EMI_OPTION: headers.indexOf('EMI_OPTION')
    };
      ...
      if (rowExtId === extId && (userEmailInput === null || userEmailInput === rowEmail)) {
        ...
        if (status === 'approved') {
          ...
          let userData = {
            ...
            emi_option: idx.EMI_OPTION !== -1 ? String(data[i][idx.EMI_OPTION]).toUpperCase() === "TRUE" : false,
          ...
          if (!otpRequired) {
            ...
            return ContentService.createTextOutput(JSON.stringify({ 
              ...
              is_admin: userData.is_admin, profile_visible: userData.profile_visible, renewal_visible: userData.renewal_visible, digital_discount: userData.digital_discount, emi_option: userData.emi_option
            })).setMimeType(ContentService.MimeType.JSON);
          }
